...
def export_midi(arrangement, filename='song.mid', arp: bool = False, tempo=500000, ticks_per_beat=480, velocity=64):
    # create file
    ...
    for chord in arrangement.progression.chords:
        if chord is None:
            chord_track.append(mido.Message(
                'note_on', note=0, velocity=0, time=note_duration))
            continue
        ...
        if (arp):
            ...
            for i, note in enumerate(midi_notes):
                ...
                # turn on notes
                chord_track.append(mido.Message(
                    'note_on', note=int(note), velocity=velocity, time=0))
                # turn off notes
                chord_track.append(mido.Message(
                    'note_off', note=int(note), velocity=0, time=step))
        else:
            ...
            for i, note in enumerate(midi_notes):
                # turn off notes
                chord_track.append(mido.Message('note_off', note=int(
                    note), velocity=0, time=note_duration if i == 0 else 0))

    # bass stuff
    for note in arrangement.bassline.notes:
        if note is None:
            bass_track.append(mido.Message('note_on', note=0,
                              velocity=0, time=note_duration))
            continue
        midi_note = translate_midi_bass(note)
        # turn on and off
        bass_track.append(mido.Message('note_on', note=int(
            midi_note), velocity=velocity, time=0))
        bass_track.append(mido.Message('note_off', note=int(
            midi_note), velocity=0, time=note_duration))
...
def translate_midi_chord(chord: Chord):
    ...
    intervals = NOTE_POSITIONS[chord.quality.value] + \
        SEVENTH_POSITIONS[chord.seventhType.value]
